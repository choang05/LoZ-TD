...
public class CameraController : MonoBehaviour
{
	...
	void LateUpdate  ()
    {
        ...
        if (canRotate && Input.GetButton("Fire3"))
        { 
			float horizontal = Input.GetAxis("Mouse X"); // The horizontal movement of the mouse.						
			float vertical = Input.GetAxis("Mouse Y"); // The vertical movement of the mouse.
			...
			else if(horizontal < 0 && horizontal < -Math.Abs(vertical))
			{
				myTransform.RotateAround(target.transform.position,new Vector3(0,1,0),-camRotationSpeed*Time.deltaTime);
				y = myTransform.eulerAngles.y;
			}
